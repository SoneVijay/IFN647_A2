"""
IFN647 Assignment 2 - Grid Search for Task2_GapK Parameters (unsupervised).

Selects MAX_K and LAM for the gap-based automatic K detection KL model.
MIN_K is fixed at 3 (always use at least 3 pseudo-relevant documents).
...
def _build_rm(query_tf, ranked_bm25, collection, coll_freq, filt_size, prf_k, lam):
...
def grid_search(stopword_file=None):
    ...
    combos = list(product(GRID['max_k'], GRID['lam']))
    total  = len(combos)
    print(f"\nSearching {total} combinations (min_k={MIN_K} fixed) ...\n")

    hdr = f"{'max_k':>6} {'lam':>5}  {'IDF quality':>11}  {'mean k':>6}"
    print(hdr)
    print("-" * len(hdr))
    ...
    for i, (max_k, lam) in enumerate(combos, 1):
        idf_scores = []
        detected_ks = []
        for pc in precomp.values():
            ...
            rm = _build_rm(pc['query_tf'], pc['ranked_bm25'], pc['collection'],
                           pc['coll_freq'], pc['filt_size'], k, lam)
            if rm:
        ...
        print(f"{max_k:>6} {lam:>5.2f}  {mean_idf:>11.4f}  {mean_k:>6.1f}  [{i}/{total}]{marker}")

        if mean_idf > best_idf:
            best_idf    = mean_idf
            best_params = dict(max_k=max_k, lam=lam, idf_quality=mean_idf, mean_k=mean_k)

    best_lam  = best_params['lam']
    best_maxk = best_params['max_k']

    lam_idf = {}
    print(f"\n--- Lambda sensitivity (max_k={best_maxk}) ---")
    ...
    maxk_idf = {}
    print(f"\n--- MAX_K sensitivity (lam={best_lam:.2f}) ---")
    ...
    print("\n" + "=" * 60)
    print("Best parameters (GapK, no relevance judgements used):")
